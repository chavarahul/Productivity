...
const Page = () => {
    ...
    const handlePage = async (e: any) => {
        setShow(false)
        e.preventDefault()
        const res = await axios.post('/api/ai', { lang: tech })
        console.log(res)
                            ...
                            <div className="w-full h-[10%]  flex items-center mt-20 ">
                                <div className=" w-full h-[30%]">
                                    <h1 className={`${poppin.className} text-white space text-center text-xl font-semibold mt-5`}>
                                        Get Most used <span className='textColor'> AI sites</span> and <span className='textColor'>Websites</span> for specific problem statments..
                                    </h1>
                                </div>
                            </div>
